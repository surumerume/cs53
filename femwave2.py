...
class Femwave:
    def __init__(self,foldername,rate,length,density,section_area,tension,d1,d3,young,moment):
        ...
        self.Mc = np.zeros( (self.M,self.M) )     #M*M 微分なし,微分なし
        self.M1 = np.zeros( (self.M-1,self.M-1) ) #(1,...,M)*(1,...,M) 微分なし,微分なし
        self.M2 = np.zeros( (self.M-1,self.M-1) ) #(0,...,M-1)*(0,...,M-1) 微分なし,微分なし
        self.M3 = np.zeros( (self.M-1,self.M-1) ) #(0,...,M-1)*(1,...,M) 微分なし,微分
        self.M4 = np.zeros( (self.M-1,self.M-1) ) #(1,...,M)*(0,...,M-1) 微分なし,微分
        self.M5 = np.zeros( (self.M-1,self.M-1) ) #(1,...,M)*(0,...,M-1) 微分,微分なし
        self.M6 = np.zeros( (self.M-1,self.M-1) ) #(0,...,M-1)*(1,...,M) 微分,微分なし
            ...
            for j in range(self.M):
                if i==j: #重なり
                    if i==0: #はじっこ
                        ...
                        self.Ma[i][j] = 0.0
                        self.Mb[i][j] = 0.0
                        self.Mc[i][j] = 2.0*self.h/3.0 
                elif i-1==j and i>=0: #かたっぽ 
                    self.Ma[i][j] = -1.0/2.0
                    self.Mb[i][j] = 1.0/2.0
                    self.Mc[i][j] = self.h/6.0
                    ...
                    self.Mb[i][j] = 0.0
                    self.Mc[i][j] = 0.0
        self.M1[:,:] = self.Mc[1:self.M,1:self.M]
        self.M2[:,:] = self.Mc[0:self.M-1,0:self.M-1]
        self.M3[:,:] = self.Mb[0:self.M-1,1:self.M]
        self.M4[:,:] = self.Mb[1:self.M,0:self.M-1] 
        self.M5[:,:] = self.Ma[1:self.M,0:self.M-1]
        self.M6[:,:] = self.Ma[0:self.M-1,1:self.M]
        #係数
        ...
        #    u(n+1)     v(n+1)    q      Du      DDu    Du(-)
        # | (1/dt)M1  (-1/2)M1    0       0       0       0    |
        # |    0       co3*M1   co1*M5 co2*M5     0    co5*M5  |
        # |    0          0       M2      0    (1/2)M6    0    |
        # |    M3         0       0      -M2      0       0    |
        # |    0          0       0      -M4      M1      0    |
        # |    M3         0       0       0       0      -M2   |
        ...
        for i in range(self.size):
            for j in range(self.size):
                if i<(self.M-1): #1行目
                    ...
                    elif (self.M-1)<=j and j<2*(self.M-1): #2列目v
                        self.co_left[i][j] = (-1.0/2.0)*self.M1[i][j-(self.M-1)]
                        self.co_right[i][j] = (1.0/2.0)*self.M1[i][j-(self.M-1)]
                elif i<2*(self.M-1): #2行目
                    if (self.M-1)<=j and j<2*(self.M-1): #2列目v
                        self.co_left[i][j] = co3*self.M1[i-(self.M-1)][j-(self.M-1)]
                        self.co_right[i][j] = co4*self.M1[i-(self.M-1)][j-(self.M-1)]
                    elif 2*(self.M-1)<=j and j<3*(self.M-1): #3列目q
                        self.co_left[i][j] = co1*self.M5[i-(self.M-1)][j-2*(self.M-1)]
                    elif 3*(self.M-1)<=j and j<4*(self.M-1): #4列目Du
                        self.co_left[i][j] = co2*self.M5[i-(self.M-1)][j-3*(self.M-1)] 
                    elif 5*(self.M-1)<=j and j<6*(self.M-1): #6列目Du(-) 
                        self.co_left[i][j] = co5*self.M5[i-(self.M-1)][j-5*(self.M-1)]
                elif i<3*(self.M-1): #3行目
                    if 2*(self.M-1)<=j and j<3*(self.M-1): #3列目
                        self.co_left[i][j] = self.M2[i-2*(self.M-1)][j-2*(self.M-1)]
                    elif 4*(self.M-1)<=j and j<5*(self.M-1): #5列目
                        self.co_left[i][j] = (1.0/2.0)*self.M6[i-2*(self.M-1)][j-4*(self.M-1)]
                elif i<4*(self.M-1): #4行目
                    if j<(self.M-1): #1列目
                        self.co_left[i][j] = self.M3[i-3*(self.M-1)][j]
                        self.co_right[i][j] = -self.M3[i-3*(self.M-1)][j]
                    elif 3*(self.M-1)<=j and j<4*(self.M-1): #4列目
                        self.co_left[i][j] = -self.M2[i-3*(self.M-1)][j-3*(self.M-1)]
                elif i<5*(self.M-1): #5行目
                    if 3*(self.M-1)<=j and j<4*(self.M-1): #4列目
                        self.co_left[i][j] = -self.M4[i-4*(self.M-1)][j-3*(self.M-1)]
                    elif 4*(self.M-1)<=j and j<5*(self.M-1): #5列目
                        self.co_left[i][j] = self.M1[i-4*(self.M-1)][j-4*(self.M-1)]
                elif i<6*(self.M-1): #6行目
                    if j<(self.M-1): #1列目u
                        self.co_left[i][j] = self.M3[i-5*(self.M-1)][j]
                        self.co_right[i][j] = self.M3[i-5*(self.M-1)][j]
                    elif 5*(self.M-1)<=j and j<6*(self.M-1): #6列目Du(-)
                        self.co_left[i][j] = -self.M2[i-5*(self.M-1)][j-5*(self.M-1)]
                else:
                   print("error")
        #その他行列
    ...
    def calc_tension(self):
        ...
        tun[1:self.M] = self.uvn[:self.M-1]
        for i in range(self.M-1):
